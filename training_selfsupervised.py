...
def train(model, train_dataloader, epochs, lr, steps_til_summary, epochs_til_checkpoint, model_dir, loss_fn,
    ...
    with tqdm(total=len(train_dataloader) * epochs - start_epoch) as pbar:
        ...
        for epoch in range(start_epoch, epochs):
            gamma = (1e-8)*((epoch - 75000 + 1) ** 2)
            # schedule beta
            if epoch < 75000:  # after pretraining on the boundary value
                gamma = 5
            else:
                if gamma >= 5:
                    gamma = 5
            ...
            for step, (model_input, gt) in enumerate(train_dataloader):
                ...
                losses = loss_fn(model_output, gt, gamma)
            ...
            if not total_steps % steps_til_summary:
                tqdm.write("Epoch %d, Total loss %0.6f, iteration time %0.6f, gamma %0.4f" % (epoch, train_loss, time.time() - start_time, gamma))
