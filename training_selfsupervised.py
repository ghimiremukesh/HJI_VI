...
def train(model, train_dataloader, epochs, lr, steps_til_summary, epochs_til_checkpoint, model_dir, loss_fn,
          ...
          loss_schedules=None, validation_fn=None, load_dir=None, start_epoch=0):
    ...
    with tqdm(total=len(train_dataloader) * epochs) as pbar:
        train_losses = []
        for epoch in range(epochs):  # change range(start_epoch, epochs) to range(epochs)
        ...
        torch.save(model.state_dict(),
                   os.path.join(checkpoints_dir, 'model_final.pth'))   # replace final with epoch number
