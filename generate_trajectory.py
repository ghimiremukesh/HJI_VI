...
# scale the angle [-1.2pi, 1.2pi]  # deepreach method
alpha_angle = 1.2 * math.pi

ckpt_path = '/home/doi5/HJI_Research/HJI_new/chkpoints/air3D_ckpt.pth'
activation = 'sine'
...
#initial state
x0 = torch.tensor([0.5, -0.5, 0.4208], requires_grad=True)  # -0.25,0.25,.4208(final state)/(-0.1767, -0.3221, 0.4208)
v = 0.75
N_num = 100
dt = 1/N_num
...
for i in range(N_num):
    ...
    dudx[..., 2] = dudx[..., 2] / (alpha_angle)   # angle is already multiplied by pi above
    ...
    ## dynamics using continuous de (scipy.odeint)
    ...
    x = x_cur + dynamics_discrete(x[..., 1:].detach().cpu().numpy().squeeze(),
                                  u.detach().cpu().numpy(), d.detach().cpu().numpy()) * dt
    xs.append(x)
...
import scipy.io
save_path = 'ground_truth_data_discrete_' + str(N_num) + '.mat'
save_dict = {'x1': x1,
             'x2': x2,
             'x3': x3,
             'u': us,
...
### BELOW IS FOR DEEPREACH'S VALUE PLOTS
angle_alpha = 1.2
...
check_BRT_path = '/home/doi5/HJI_Research/HJI_new/chkpoints/analytical_BRT_air3D.mat'
