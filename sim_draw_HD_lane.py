...
class VisUtils:
    ...
    def __init__(self):
        ...
        load_path = 'examples/vehicle/data_test_a_a_1.mat'
        ...
        for k in range(steps - 1):
            ...
            for i in range(n_agents):
                ...
                if i == 1:
                    ...
                    pos = (pos_x, 70 - pos_y)  # car position
                    orientation = -180 + math.atan((pos_x_new - pos_x_old)/(pos_y_new - pos_y_old)) * 180 / math.pi
                ...
                # self.blit_alpha(self.screen, self.car_image.convert_alpha(),(pixel_pos_car[0] - size_car[0] / 2, pixel_pos_car[1] - size_car[1] / 2), 11 * k)

                self.screen.blit(car_image, (pixel_pos_car[0] - size_car[0] / 2, pixel_pos_car[1] - size_car[1] / 2))
                # self.screen.blit(self.car_image[i], (pixel_pos_car[0] - size_car[0] / 2, pixel_pos_car[1] - size_car[1] / 2))
                time.sleep(0.05)
    ...
    def draw_axes(self):
        ...
        pg.draw.line(self.screen, LIGHT_GREY, self.c2p((100, 35)),
                     self.c2p((-50, 35)), self.car_image[1].get_size()[0] * 4)
    ...
    def generate(self, data):
        ...
        while time <= 3.0:
            ...
            X_bar[2][j] = (time - t[0][i - 1]) * (X[5][i] - X[5][i - 1]) / (t[0][i] - t[0][i - 1]) + X[5][i - 1]
            X_bar[3][j] = (time - t[0][i - 1]) * (X[4][i] - X[4][i - 1]) / (t[0][i] - t[0][i - 1]) + X[4][i - 1]
            time = time + 0.1
            j += 1
