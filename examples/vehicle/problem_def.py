...
class setup_problem(problem_prototype):
    def __init__(self):
        ...
        # [18.87293787 21.09137217 16.38732673 18.7859253]
        self.X0_lb = np.array([[18.87293787], [21.09137217], [16.38732673], [18.7859253]])
        self.X0_ub = np.array([[18.87293787], [21.09137217], [16.38732673], [18.7859253]])  # 20.1 19

        # self.X0_lb = np.array([[15.], [18.], [15.], [18.]])
        # self.X0_ub = np.array([[20.], [25.], [20.], [25.]])  # 20.1 19

        # self.X0_lb = np.array([[15.], [18.], [20.], [22.]])
        # self.X0_ub = np.array([[18.6], [18.], [25.], [25.]])  # 20.1 19

        # self.X0_lb = np.array([[20.], [22.], [15.], [18.]])
        # self.X0_ub = np.array([[25.], [25.], [20.], [22.]])  # 20.1 19

        # self.X0_lb = np.array([[15.], [18.], [20.], [22.]])
        # self.X0_ub = np.array([[20.], [22.], [25.], [25.]])  # 20.1 19

        # self.X0_lb = np.array([[15.1], [18.], [15.], [18.]])
        # self.X0_ub = np.array([[15.1], [18.], [15.], [18.]])  # 20.1 19
    ...
    def aug_dynamics(self, t, X_aug):
        ...
        # x1_in = (x1 - self.R1 / 2 + self.theta1 * self.W2 / 2) * 5  # 3
        # x1_out = -(x1 - self.R1 / 2 - self.W2 / 2 - self.L1) * 5
        # x2_in = (x2 - self.R2 / 2 + self.W1 / 2) * 5
        # x2_out = -(x2 - self.R2 / 2 - self.W1 / 2 - self.L2) * 5
        #
        # Collision_F_x1 = self.beta * torch.sigmoid(x1_in) * torch.sigmoid(x1_out) * torch.sigmoid(x2_in) * torch.sigmoid(
        #     x2_out)
        #
        # Collision_F_x_sum1 = torch.sum(Collision_F_x1)
        # Collision_F_x_sum1.requires_grad_()
        #
        # x1_in = (x1 - self.R1 / 2 + self.W2 / 2) * 5  # 3
        # x1_out = -(x1 - self.R1 / 2 - self.W2 / 2 - self.L1) * 5
        # x2_in = (x2 - self.R2 / 2 + self.theta2 * self.W1 / 2) * 5
        # x2_out = -(x2 - self.R2 / 2 - self.W1 / 2 - self.L2) * 5
        #
        # Collision_F_x2 = self.beta * torch.sigmoid(x1_in) * torch.sigmoid(x1_out) * torch.sigmoid(x2_in) * torch.sigmoid(
        #     x2_out)
        #
        # Collision_F_x_sum2 = torch.sum(Collision_F_x2)
        # Collision_F_x_sum2.requires_grad_()
        #
        # dL1dx1 = torch.autograd.grad(Collision_F_x_sum1, x1, create_graph=True)[0].detach().numpy()
        # dL1dx2 = torch.autograd.grad(Collision_F_x_sum1, x2, create_graph=True)[0].detach().numpy()
        # dL2dx1 = torch.autograd.grad(Collision_F_x_sum2, x1, create_graph=True)[0].detach().numpy()
        # dL2dx2 = torch.autograd.grad(Collision_F_x_sum2, x2, create_graph=True)[0].detach().numpy()
        ...
        # L1 = U1 ** 2 + Collision_F_x1.detach().numpy()
        # L2 = U2 ** 2 + Collision_F_x2.detach().numpy()
