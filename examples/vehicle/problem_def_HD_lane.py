...
class setup_problem(problem_prototype):
    def __init__(self):
        ...
        self.threshold = 3  # 3 * math.sqrt(2)
    ...
    def Omega_star(self, X_aug):
        ...
        # If we keep collision function L in the value cost function V, we consider dH/du = 0 and get the U*
        A = X_aug[2 * self.N_states:3 * self.N_states]
        Omega1 = np.matmul(self.B1.T, A) / (2 * 100)
        A = X_aug[5 * self.N_states:6 * self.N_states]
        Omega2 = np.matmul(self.B1.T, A) / (2 * 100)
        ...
        # Omega1[np.where(Omega1 > 0)] = max_acc
        # Omega1[np.where(Omega1 < 0)] = min_acc
        # Omega2[np.where(Omega2 > 0)] = max_acc
        # Omega2[np.where(Omega2 < 0)] = min_acc
    ...
    def aug_dynamics(self, t, X_aug):
        ...
        R = torch.tensor(self.R1, requires_grad=True, dtype=torch.float32)

        dist_diff = torch.sigmoid(-(torch.sqrt(((R - dx2) - dx1) ** 2 + (dy2 - dy1) ** 2) - self.threshold) * 5)
        ...
        L1 = U1 ** 2 + Collision_F_x.detach().numpy() + 100 * Omega1 ** 2
        L2 = U2 ** 2 + Collision_F_x.detach().numpy() + 100 * Omega2 ** 2
