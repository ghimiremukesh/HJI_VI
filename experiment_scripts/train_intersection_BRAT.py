...
p.add_argument('--tMax', type=float, default=1.5, required=False, help='End time of the simulation')
p.add_argument('--num_hl', type=int, default=5, required=False, help='The number of hidden layers')
...
'''
Using HJI VI(BRAT) to train the value network
'''
dataset = dataio.IntersectionBRAT(numpoints=10000,
...
# Define the loss
'''
loss definition for HJI VI(BRAT)
'''
loss_fn = loss_functions.initialize_intersection_BRAT(dataset, opt.minWith)

root_path = os.path.join(opt.logging_root, 'experiment_check_BRAT_test/')
