...
#
# # supervised learning
supervised_dataset = dataio.IntersectionHJI_Supervised(seed=opt.seed)

supervised_dataloader = DataLoader(supervised_dataset, shuffle=True, batch_size=opt.batch_size, pin_memory=True, num_workers=0)

# hybrid model
dataset = dataio.IntersectionHJI_Hybrid(numpoints=30000, tMin=opt.tMin, tMax=opt.tMax,
...
#
loss_fn_supervised = loss_functions.initialize_intersection_HJI_supervised(dataset, opt.minWith)

loss_fn = loss_functions.initialize_intersection_HJI_hyrid(dataset, opt.minWith)

root_path = os.path.join(opt.logging_root, 'experiment_check_HJI_tanh_supervised_a_a/')

# hybrid training: supervised learning + HJI
# training.train(model=model, train_dataloader=dataloader, epochs=opt.num_epochs, lr=opt.lr,
#                steps_til_summary=opt.steps_til_summary, epochs_til_checkpoint=opt.epochs_til_ckpt,
#                model_dir=root_path, loss_fn=loss_fn, clip_grad=opt.clip_grad, use_lbfgs=opt.use_lbfgs,
#                validation_fn=None, start_epoch=opt.checkpoint_toload)

# pretrain: supervised learning, train: HJI
training.train(model=model, train_dataloader=dataloader, train_dataloader_supervised=supervised_dataloader,
               epochs=opt.num_epochs, lr=opt.lr, steps_til_summary=opt.steps_til_summary,
               epochs_til_checkpoint=opt.epochs_til_ckpt, model_dir=root_path, loss_fn=loss_fn,
               loss_fn_supervised=loss_fn_supervised, clip_grad=opt.clip_grad,
