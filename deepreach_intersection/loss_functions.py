...
def initialize_intersection_HJI(dataset, minWith):
    def intersection_hji(model_output, gt):
        ...
        x11_in = ((d11 - R1 / 2 + theta1 * W2 / 2) * 5).squeeze().reshape(-1, 1).cuda()
        x11_out = (-(d11 - R1 / 2 - W2 / 2 - L1) * 5).squeeze().reshape(-1, 1).cuda()
        x12_in = ((d12 - R2 / 2 + theta2 * W1 / 2) * 5).squeeze().reshape(-1, 1).cuda()
        x12_out = (-(d12 - R2 / 2 - W1 / 2 - L2) * 5).squeeze().reshape(-1, 1).cuda()
        ...
        # calculate the collision area lower and upper bounds
        x21_in = ((d21 - R1 / 2 + theta1 * W2 / 2) * 5).squeeze().reshape(-1, 1).cuda()
        x21_out = (-(d21 - R1 / 2 - W2 / 2 - L1) * 5).squeeze().reshape(-1, 1).cuda()
        x22_in = ((d22 - R2 / 2 + theta2 * W1 / 2) * 5).squeeze().reshape(-1, 1).cuda()
        x22_out = (-(d22 - R2 / 2 - W1 / 2 - L2) * 5).squeeze().reshape(-1, 1).cuda()
        ...
        return {'dirichlet': torch.abs(dirichlet).sum() / 20,  # 1e4
                'diff_constraint_hom': torch.abs(diff_constraint_hom).sum() / 100}
    return intersection_hji
...
def initialize_intersection_HJI_supervised(dataset, minWith):
    def intersection_hji(model_output, gt):
        ...
        # return {'values_difference': torch.abs(values_difference).sum() / 30,  # 1e4
        #         'costates_difference': torch.abs(costates_difference).sum() / 100,
        #         'diff_constraint_hom': torch.abs(diff_constraint_hom).sum() / 30}
    return intersection_hji
...
def initialize_intersection_HJI_hyrid(dataset, minWith):
    def intersection_hji(model_output, gt):
        ...
        return {'values_difference': torch.abs(values_difference).sum() / 40,  # 1e4
                'costates_difference': torch.abs(costates_difference).sum() / 120,
                'dirichlet': torch.abs(dirichlet).sum() / 15,  # 1e4
