...
def solve_bvp(fun, bc, x, y, p=None, S=None, fun_jac=None, bc_jac=None,
    ...
    while True:
        ...
        # axs.plot(y[0, :], y[2, :])
        #
        # train1 = patches.Rectangle((35 - problem.theta1 * 0.75, 35 - problem.theta2 * 0.75), 3 + problem.theta1 * 0.75 + 0.75,
        #                            3 + problem.theta2 * 0.75 + 0.75, linewidth=1, edgecolor='k', facecolor='none')
        ...
        # axs.set_ylabel('d2')
        #
        # p1 = [0, 35 - problem.theta1 * 0.75]
        # p2 = [0, 35 - problem.theta2 * 0.75]
        # axs.plot(p1, p2, '-r')
        #
        # # axs.plot(x, y[4, :], label='$\lambda_{11}(1)$')
        # # axs.plot(x, y[10, :], label='$\lambda_{22}(1)$')
        # # plt.legend()
...
def y_modify(m, y, x):
    ...
    lambda_change_1 = False
    lambda_change_2 = False
    lambda_change_3 = False
    l1 = problem.R1 / 2 - problem.theta1 * problem.W1 / 2
    u1 = problem.R1 / 2 + problem.W1 / 2 + problem.L1
    l2 = problem.R2 / 2 - problem.theta2 * problem.W2 / 2
    u2 = problem.R2 / 2 + problem.W2 / 2 + problem.L2
    ...
    for i in range(m - 1):
        ...
        a2 = y[2, i]
        b2 = y[2, i + 1]
        itea_lower = max((l1 - a1)/(b1 - a1), (l2 - a2)/(b2 - a2), 0)
        itea_upper = min((u1 - a1)/(b1 - a1), (u2 - a2)/(b2 - a2), 1)

        if itea_lower == 0 and 0 < itea_upper <= 1 and collision == False:
            Istart = i - 1
            Tstart = x[Istart]
            collision = True
            lambda_change_1 = True

        if itea_upper == 1 and 0 <= itea_lower < 1 and collision == True:
            Iend = i + 2
            Tend = x[Iend]
            lambda_change_2 = True

        if 0 < itea_lower < itea_upper < 1 and collision == False:
            Istart = i
            Tstart = x[Istart]
            Iend = i + 1
            Tend = x[Iend]
            lambda_change_3 = True
            break

        if itea_lower > itea_upper:
            continue

    if lambda_change_1 is True:
        # depend on the boundary condition to choose which lambda shape can be changed
        # three points definition, one of them are inside the collision zone, the rest are outside the collision zone
        m1 = y[0, Istart]
        n1 = y[0, Istart + 1]
        p1 = y[0, Istart + 2]
        ...
        gamma1 = ((l1 - m1) * (n2 - l2) - (l2 - m2) * (n1 - l1)) / ((n1 - m1) * (u2 - l2))
        gamma2 = ((l2 - m2) * (n1 - l1) - (l1 - m1) * (n2 - l2)) / ((n2 - m2) * (u1 - l1))
        gamma3 = ((u1 - n1) * (p2 - l2) - (l2 - n2) * (p1 - u1)) / ((p1 - n1) * (u2 - l2))
        gamma4 = ((u2 - n2) * (p1 - l1) - (l1 - n1) * (p2 - u2)) / ((p2 - n2) * (u1 - l1))
        ...
        if 0 < gamma1 < 1 and 0 < gamma4 < 1:
            condition1 = True

        if 0 < gamma1 < 1 and 0 < gamma3 < 1:
            condition2 = True

        if 0 < gamma2 < 1 and 0 < gamma4 < 1:
            condition3 = True

        if 0 < gamma2 < 1 and 0 < gamma3 < 1:
            condition4 = True

        y[4, Istart:m] = problem.alpha
        y[10, Istart:m] = problem.alpha  # 10
        y[5, -1] = -2 * (y[1, -1] - y[1, 0])
        y[11, -1] = -2 * (y[3, -1] - y[3, 0])  # 11

        # condition 1
        if condition1 is True:
            y[4, 0:Istart] = -problem.beta / y[1, Istart]
            y[10, 0:Istart] = problem.beta / y[3, Istart]  # 10
            k1 = -problem.beta / y[1, Istart]
            k2 = problem.beta / y[3, Istart]
            for i in range(Istart, m - 1):
                y[5, i] = -problem.alpha * (x[i] - x[-1]) + y[5, -1]
                y[11, i] = -problem.alpha * (x[i] - x[-1]) + y[11, -1]  # 11
            S1, S2 = y[5, Istart], y[11, Istart]  # 11
            for i in range(Istart):
                y[5, i] = -k1 * (x[i] - Tstart) + S1
                y[11, i] = -k2 * (x[i] - Tstart) + S2  # 11

        # condition 2
        if condition2 is True:
            y[4, Istart:Iend + 1] = problem.beta / y[1, Iend]
            y[4, 0:Istart] = problem.beta / y[1, Iend] - problem.beta / y[1, Istart] + problem.alpha
            k1 = problem.beta / y[1, Iend]
            k2 = problem.beta / y[1, Iend] - problem.beta / y[1, Istart] + problem.alpha
            for i in range(Iend, m - 1):
                y[5, i] = -problem.alpha * (x[i] - x[-1]) + y[5, -1]
            for i in range(Istart, Iend):
                y[5, i] = -k1 * (x[i] - Tend) + y[5, Iend]
            for i in range(Istart):
                y[5, i] = -k2 * (x[i] - Tstart) + y[5, Istart]
            y[10, 0:Istart] = problem.alpha  # 10
            for i in range(m - 1):
                y[11, i] = -problem.alpha * (x[i] - x[-1]) + y[11, -1]  # 11

        # condition 3
        if condition3 is True:
            y[10, Istart:Iend + 1] = problem.beta / y[3, Iend]  # 10
            y[10, 0:Istart] = problem.beta / y[3, Iend] - problem.beta / y[3, Istart] + problem.alpha  # 10
            k1 = problem.beta / y[3, Iend]
            k2 = problem.beta / y[3, Iend] - problem.beta / y[3, Istart] + problem.alpha
            for i in range(Iend, m - 1):
                y[11, i] = -problem.alpha * (x[i] - x[-1]) + y[11, -1]  # 11
            for i in range(Istart, Iend):
                y[11, i] = -k1 * (x[i] - Tend) + y[11, Iend]  # 11
            for i in range(Istart):
                y[11, i] = -k2 * (x[i] - Tstart) + y[11, Istart]  # 11
            y[4, 0:Istart] = problem.alpha
            for i in range(m - 1):
                y[5, i] = -problem.alpha * (x[i] - x[-1]) + y[5, -1]
    ...
    if lambda_change_2 is True:
        # depend on the boundary condition to choose which lambda shape can be changed
        # four points definition, two of them are inside the collision zone, the rest are outside the collision zone
        m1 = y[0, Istart]
        n1 = y[0, Istart + 1]
        p1 = y[0, Iend - 1]
        q1 = y[0, Iend]
        ...
        gamma1 = ((l1 - m1) * (n2 - l2) - (l2 - m2) * (n1 - l1)) / ((n1 - m1) * (u2 - l2))
        gamma2 = ((l2 - m2) * (n1 - l1) - (l1 - m1) * (n2 - l2)) / ((n2 - m2) * (u1 - l1))
        gamma3 = ((u1 - p1) * (q2 - l2) - (l2 - p2) * (q1 - u1)) / ((q1 - p1) * (u2 - l2))
        gamma4 = ((u2 - p2) * (q1 - l1) - (l1 - p1) * (q2 - u2)) / ((q2 - p2) * (u1 - l1))
    ...
    if lambda_change_3 is True:
        # depend on the boundary condition to choose which lambda shape can be changed
        # two points definition, which are outside the collision zone
        m1 = y[0, Istart]
        n1 = y[0, Iend]
        ...
        gamma1 = ((l1 - m1) * (n2 - l2) - (l2 - m2) * (n1 - l1)) / ((n1 - m1) * (u2 - l2))
        gamma2 = ((l2 - m2) * (n1 - l1) - (l1 - m1) * (n2 - l2)) / ((n2 - m2) * (u1 - l1))
        gamma3 = ((u1 - m1) * (n2 - l2) - (l2 - m2) * (n1 - u1)) / ((n1 - m1) * (u2 - l2))
        gamma4 = ((u2 - m2) * (n1 - l1) - (l1 - m1) * (n2 - u2)) / ((n2 - m2) * (u1 - l1))
